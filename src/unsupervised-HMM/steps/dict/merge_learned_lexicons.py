...
def WriteEditsAndSummary(args, learned_lexicon, ref_lexicon, pd_lexicon, g2p_lexicon, counts, stats, stats_summed):
    ...
    print("Average num. prons per word in the learned lexicon is {}".format(
        float(num_prons_tot) / float(len(learned_lexicon))), file=sys.stderr)
    ...
    print(
        "We have acoustic evidence for {} out of {} in-vocab (w.r.t the reference lexicon) words from the acoustic training data.".format(
            num_ivs, len(ref_lexicon)), file=sys.stderr)
    ...
    print(
        'We have acoustic evidence for {} out of {} OOV (w.r.t the reference lexicon) words from the acoustic training data.'.format(
            num_oovs_with_acoustic_evidence, num_oovs), file=sys.stderr)
