...
function main_function {
    ...
    while read LINE
    do
        video_name=$(echo $(basename $LINE))
        count=$(($count+1))
        echo "Processing " $video_name " " $count"/"$doc_len"."
        # extract mfcc feats
        echo "Extract mfcc feats..."
        echo "$video_name ${LINE}.wav" > $tmp_folder/tmp.scp
        compute-mfcc-feats --sample-frequency=16000 --channel=0 scp:$tmp_folder/tmp.scp ark,t,scp:$tmp_folder/tmp.$video_name.13.ark,$tmp_folder/tmp.$video_name.13.scp
        wait $!
        ...
        wait $!
        apply-cmvn --norm-vars=false ark:$tmp_folder/tmp.$video_name.cmvn.ark scp:$tmp_folder/tmp.$video_name.delta.scp ark,t,scp:$feat_dir/$video_name.mfcc.cmvn.ark,$feat_dir/$video_name.mfcc.cmvn.scp
        wait $!
        rm $tmp_folder/tmp*
    done < $list_name
}
