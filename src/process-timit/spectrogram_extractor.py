...
def extract_spec(wav_path):
    ...
    D = librosa.core.stft(data, n_fft=n_fft, hop_length=hop_length, win_length=win_length, window="hamming",
                          center=False)
...
def griffin_lim(stftm_matrix, shape, min_iter=20, max_iter=50, delta=20):
    ...
    for i in range(max_iter):
        if i >= min_iter and (i - min_iter) % delta == 0:
            y_iter.append((y, i))
        stft_matrix = librosa.core.stft(y, n_fft=n_fft, hop_length=hop_length, win_length=win_length, window="hamming",
                                        center=False)
        stft_matrix = stftm_matrix * stft_matrix / np.abs(stft_matrix)
        y = librosa.core.istft(stft_matrix, hop_length=hop_length, win_length=win_length, window="hamming",
                               center=False)
