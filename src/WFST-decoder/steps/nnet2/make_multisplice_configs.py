...
def parse_splice_string(splice_string):
    ...
    return [contexts,
            ' nnet_left_context={0};\n nnet_right_context={1}\n first_left_context={2};\n first_right_context={3}\n'.format(
                abs(max_left_context), abs(max_right_context), abs(first_left_context), abs(first_right_context))]
...
def create_config_files(output_dir, params):
    ...
    Splicing is supported only until layer {1}.""".format(params.num_hidden_layers, params.num_hidden_layers - 1,
                                                          params.splice_indexes))

    stddev = 1.0 / math.sqrt(pnorm_input_dim)
    try:
        nnet_config = [
            "SpliceComponent input-dim={0} context={1} const-component-dim={2}".format(params.total_input_dim,
                                                                                       contexts[0], params.ivector_dim),
            ...
            "AffineComponentPreconditionedOnline input-dim={0} output-dim={1} {2} learning-rate={3} param-stddev={4} bias-stddev={5}".format(
                params.lda_dim, pnorm_input_dim, params.online_preconditioning_opts, params.initial_learning_rate,
                stddev, params.bias_stddev),
            ("PnormComponent input-dim={0} output-dim={1} p={2}".format(pnorm_input_dim, pnorm_output_dim,
                                                                        pnorm_p) if pnorm_input_dim != pnorm_output_dim else "RectifiedLinearComponent dim={0}".format(
                pnorm_input_dim)),
            ...
            "AffineComponentPreconditionedOnline input-dim={0} output-dim={1} {2} learning-rate={3} param-stddev=0 bias-stddev=0".format(
                pnorm_output_dim, params.num_targets, params.online_preconditioning_opts, params.initial_learning_rate),
    ...
    for i in xrange(1,
                    params.num_hidden_layers):  # just run till num_hidden_layers-1 since we do not add splice before the final affine transform
        lines = []
        context_len = 1
        ...
        lines.append(
            ...
            % (pnorm_output_dim * context_len, pnorm_input_dim, params.online_preconditioning_opts,
               params.initial_learning_rate, stddev, params.bias_stddev))
        if pnorm_input_dim != pnorm_output_dim:
            lines.append(
                "PnormComponent input-dim=%d output-dim=%d p=%d" % (pnorm_input_dim, pnorm_output_dim, pnorm_p))
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--online-preconditioning-opts', type=str,
    ...
    parser.add_argument('--bias-stddev', type=float,
