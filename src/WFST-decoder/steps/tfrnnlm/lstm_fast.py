...
class RnnlmModel(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        self.initial = tf.reshape(tf.stack(axis=0, values=self._initial_state_single), [config.num_layers, 2, 1, size],
        ...
        test_state_out = tf.reshape(tf.stack(axis=0, values=test_output_state), [config.num_layers, 2, 1, size],
                                    name="test_state_out")
        ...
        test_logits = tf.matmul(cellout_placeholder,
                                tf.transpose(tf.nn.embedding_lookup(tf.transpose(softmax_w), test_word_out[0]))) + \
