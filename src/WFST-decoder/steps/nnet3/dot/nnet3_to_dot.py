...
def ProcessAppendDescriptor(segment, parent_node_name, affix, edge_attributes=None):
    ...
    for i in range(len(segment['arguments'])):
        ...
        dot_graph.append(
            "{0} -> {1}:{2}".format(GetDotNodeName(segment['arguments'][i])['node'], GetDotNodeName(desc_name)['node'],
    ...
    dot_string = '{0} -> {1} [tailport=s]'.format(GetDotNodeName(desc_name)['node'],
...
def ProcessSumDescriptor(segment, parent_node_name, affix, edge_attributes=None):
    ...
    for i in range(len(segment['sub_segments'])):
        ...
        part_name = "{0}{1}{2}".format(desc_name, sub_segment['name'], i)
        names.append("<{0}> part {1}".format(GetDotNodeName(part_name)['node'], i))
        dot_graph += DescriptorSegmentToDot(sub_segment, "{0}:{1}".format(desc_name, part_name),
                                            desc_name + "_" + str(i))
...
def ProcessIfDefinedDescriptor(segment, parent_node_name, affix, edge_attributes=None):
    ...
    if segment['arguments']:
        dot_graph.append(
            '{0} -> {1} [style=dotted, label="IfDefined"]'.format(GetDotNodeName(segment['arguments'][0])['node'],
                                                                  GetDotNodeName(parent_node_name)['node']))
...
def Nnet3DescriptorToDot(descriptor, parent_node_name):
    ...
    if segments:
        ...
        dot_lines.append(
            "{0} -> {1}".format(GetDotNodeName(arguments[0])['node'], GetDotNodeName(parent_node_name)['node']))
    return dot_lines
...
def Nnet3ComponentToDot(component_config, component_attributes=None):
    ...
    return ['{0} [label="{1}" {2}]'.format(GetDotNodeName(component_config['name'], is_component=True)['node'], label,
                                           attr_string)]


# input-node name=input dim=40
def Nnet3InputToDot(parsed_config):
    return ['{0} [ label="{1}\\ndim={2}"]'.format(GetDotNodeName(parsed_config['name'])['node'], parsed_config['name'],
                                                  parsed_config['dim'])]
...
def Nnet3OutputToDot(parsed_config):
    ...
    dot_graph.append('{0} [ label="{1}\\nobjective={2}"]'.format(GetDotNodeName(parsed_config['name'])['node'],
                                                                 parsed_config['name'], parsed_config['objective']))
    return dot_graph
...
def Nnet3DimrangeToDot(parsed_config):
    ...
    dot_graph.append(
        '{0} -> {1} [taillabel="dimrange({2}, {3})"]'.format(GetDotNodeName(parsed_config['input-node'])['node'],
...
def Nnet3ComponentNodeToDot(parsed_config):
    ...
    dot_graph.append(
        '{0} -> {1} [ weight=10 ]'.format(GetDotNodeName(parsed_config['component'], is_component=True)['node'],
                                          GetDotNodeName(parsed_config['name'])['node']))
    return dot_graph
