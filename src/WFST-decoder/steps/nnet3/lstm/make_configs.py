    ...
    try:
        for i in range(len(split1)):
            ...
            if len(indexes) < 1:
                ...
                raise ValueError('Warning: ' + str(
                    indexes) + ' is not a standard BLSTM mode. There should be a negative delay for the forward, and a postive delay for the backward.')
...
def MakeConfigs(config_dir, feat_dim, ivector_dim, num_targets,
    ...
    for i in range(num_lstm_layers):
        ...
        nodes.AddFinalLayer(config_lines, prev_layer_output, num_targets, ng_affine_options,
                            max_change_per_component=max_change_per_component_final, label_delay=label_delay,
