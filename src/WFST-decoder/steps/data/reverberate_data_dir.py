...
def GetArgs():
    ...
    parser.add_argument('--foreground-snrs', type=str, dest="foreground_snr_string", default='20:10:0',
                        help='When foreground noises are being added the script will iterate through these SNRs.')
    parser.add_argument('--background-snrs', type=str, dest="background_snr_string", default='20:10:0',
    ...
    parser.add_argument("--include-original-data", type=str,
...
def AddPointSourceNoise(noise_addition_descriptor,  # descriptor to store the information of the noise added
    ...
    if len(
            pointsource_noise_list) > 0 and random.random() < pointsource_noise_addition_probability and max_noises_recording >= 1:
...
def GenerateReverberationOpts(room_dict,  # the room dictionary, please refer to MakeRoomDict() for the format
    ...
    if len(rir_iso_noise_list) > 0 and random.random() < isotropic_noise_addition_probability:
        ...
        if len(isotropic_noise.noise_rspecifier.split()) == 1:
            noise_addition_descriptor['noise_io'].append(
                "wav-reverberate --duration={1} {0} - |".format(isotropic_noise.noise_rspecifier, speech_dur))
        else:
            noise_addition_descriptor['noise_io'].append(
                "{0} wav-reverberate --duration={1} - - |".format(isotropic_noise.noise_rspecifier, speech_dur))
        noise_addition_descriptor['start_times'].append(0)
        noise_addition_descriptor['snrs'].append(background_snrs.next())

    noise_addition_descriptor = AddPointSourceNoise(noise_addition_descriptor,
                                                    # descriptor to store the information of the noise added
                                                    room,  # the room selected
                                                    pointsource_noise_list,  # the point source noise list
                                                    pointsource_noise_addition_probability,
                                                    # Probability of adding point-source noises
                                                    foreground_snrs,  # the SNR for adding the foreground noises
                                                    background_snrs,  # the SNR for adding the background noises
                                                    speech_dur,  # duration of the recording
                                                    max_noises_recording
                                                    # Maximum number of point-source noises that can be added
                                                    )
...
def GenerateReverberatedWavScp(wav_scp,  # a dictionary whose values are the Kaldi-IO strings of the speech recordings
                               durations,
                               # a dictionary whose values are the duration (in sec) of the speech recordings
                               ...
                               # maximum number of point-source noises that can be added to a recording according to its duration
                               ):
    ...
    for i in range(start_index, num_replicas + 1):
        for recording_id in keys:
            ...
            reverberate_opts = GenerateReverberationOpts(room_dict,
                                                         # the room dictionary, please refer to MakeRoomDict() for the format
                                                         pointsource_noise_list,  # the point source noise list
                                                         iso_noise_dict,  # the isotropic noise dictionary
                                                         foreground_snrs,  # the SNR for adding the foreground noises
                                                         background_snrs,  # the SNR for adding the background noises
                                                         speech_rvb_probability,
                                                         # Probability of reverberating a speech signal
                                                         isotropic_noise_addition_probability,
                                                         # Probability of adding isotropic noises
                                                         pointsource_noise_addition_probability,
                                                         # Probability of adding point-source noises
                                                         speech_dur,  # duration of the recording
                                                         max_noises_recording
            ...
            if reverberate_opts == "" or i == 0:
                ...
                wav_corrupted_pipe = "{0} wav-reverberate --shift-output={1} {2} - - |".format(wav_original_pipe,
                                                                                               shift_output,
                                                                                               reverberate_opts)
...
def CreateReverberatedCopy(input_dir,
    ...
    if not os.path.isfile(input_dir + "/reco2dur"):
        ...
        data_lib.RunKaldiCommand(
            "wav-to-duration --read-entire-file={1} scp:{0}/wav.scp ark,t:{0}/reco2dur".format(input_dir,
                                                                                               read_entire_file))
    ...
    AddPrefixToFields(input_dir + "/utt2spk", output_dir + "/utt2spk", num_replicas, include_original, prefix,
                      field=[0, 1])
    ...
    if os.path.isfile(input_dir + "/utt2uniq"):
        AddPrefixToFields(input_dir + "/utt2uniq", output_dir + "/utt2uniq", num_replicas, include_original, prefix,
                          field=[0])
    else:
    ...
    if os.path.isfile(input_dir + "/segments"):
        AddPrefixToFields(input_dir + "/segments", output_dir + "/segments", num_replicas, include_original, prefix,
                          field=[0, 1])
    if os.path.isfile(input_dir + "/reco2file_and_channel"):
        AddPrefixToFields(input_dir + "/reco2file_and_channel", output_dir + "/reco2file_and_channel", num_replicas,
...
def ParseRirList(rir_set_para_array, smoothing_weight, sampling_rate=None):
    ...
    rir_parser.add_argument('--rt60', type=float, default=None,
                            help='RT60 is the time required for reflections of a direct sound to decay 60 dB.')
    rir_parser.add_argument('--drr', type=float, default=None,
...
def ParseNoiseList(noise_set_para_array, smoothing_weight, sampling_rate=None):
    ...
    noise_parser.add_argument('--noise-type', type=str, required=True,
                              help='the type of noise; i.e. isotropic or point-source',
                              choices=["isotropic", "point-source"])
    noise_parser.add_argument('--bg-fg-type', type=str, default="background",
    ...
    noise_parser.add_argument('--room-linkage', type=str, default=None,
    ...
    for noise_set in set_list:
        ...
        for noise in current_noise_list:
            if sampling_rate is not None:
                ...
                if len(noise.noise_rspecifier.split()) == 1:
                    ...
                    noise.noise_rspecifier = "{0} sox -t wav - -r {1} -t wav - |".format(noise.noise_rspecifier,
                                                                                         sampling_rate)
        ...
        pointsource_noise_list += SmoothProbabilityDistribution(current_pointsource_noise_list, smoothing_weight,
                                                                noise_set.probability)
...
def Main():
    ...
    if args.noise_set_para_array is not None:
        ...
        print(
            "Number of isotropic noises is {0}".format(sum(len(iso_noise_dict[key]) for key in iso_noise_dict.keys())))
